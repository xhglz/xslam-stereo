...
std::vector<cv::Point2f> normalizeKeyPoints(std::vector<cv::Point2f> key_points, const int& height, const int& width) {
  ...
  float scale = static_cast<float>(std::max(width, height)) / 2.0f;
...
cv::Mat resizeImage(const cv::Mat& image, const int& size, float& scale, 
                    const std::string& fn, const std::string& interpolation) {
...
std::vector<cv::Point2f> getKeyPointsInOriginalImage(const std::vector<cv::Point2f>& key_points, const float scale) {
  ...
  for (const auto& key_point : key_points) {
    key_points_in_original_image.push_back(cv::Point2f((key_point.x) / scale, (key_point.y) / scale));
